...
- =:after= advice on =bookmark-store= so the autotemp predicates
  fire for any caller, not just =bmkx-set=.
- =:around= advice on =bookmark-write-file= so the built-in save
  skips =bmkx-temp= records, just as =bmkx-write-file= does.
